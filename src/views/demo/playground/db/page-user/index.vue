<template>
  <sys-container>
    <template slot="header">
      <el-alert
        ...
        title="私有路由存储指当前路由的存储区域，
          ...
          使用 await this.$store.dispatch('system/db/databasePage', { user: true }) 获得存储实例进行操作，
    ...
    <el-row>
      <el-col :span="12">
        <p class="sys-mt-0">增加不重复字段</p>
        <el-button @click="handleSetRandom">增加</el-button>
        <p>增加自定义字段</p>
        <el-input v-model="keyNameToSet" placeholder="字段名" class="sys-mr-5" style="width: 100px;"/>
        <el-input v-model="valueToSet" placeholder="值" class="sys-mr-5" style="width: 100px;"/>
      ...
      <el-col :span="12">
        <sys-highlight :code="dataDisplay"/>
      </el-col>
    </el-row>
  </sys-container>
</template>
